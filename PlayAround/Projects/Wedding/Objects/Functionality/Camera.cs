...
namespace PlayAround
{
    ...
    public class Camera2D : GameComponent, ICamera2D
    {
        ...
        {
            //float diff = 1f + (1f - zoom);
            var matrix = Matrix.Invert(Transform);
            Vector2 vec = Vector2.Transform(new Vector2(rect.X, rect.Y), matrix);
            return new Rectangle((int)vec.X, (int)vec.Y, rect.Width, rect.Height);
            //return new Rectangle((int)(vec.X/diff), (int)(vec.Y/diff), (int)(rect.Width/diff), (int)(rect.Height/diff));
        }
    }
}
